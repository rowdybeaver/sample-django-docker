...
class HostPort:
    def __init__(self, hostport_str):
        badchars = set(hostport_str.lower()).difference(set('abcdefghijklmnopqrstuvwxyz0123456789.-:'))
        if badchars:
            msg = "Unacceptable characters in host: '{}'".format(str(list(badchars)))
            logger.fatal(msg)
            raise ValueError(msg)
        if ':' not in hostport_str:
            msg = "Expecting a host and port specification (e.g. 'host:port'). Found '{}'".format(hostport_str)
            raise ValueError(msg)
    ...
    def test_socket(self, timeout):
        ...
        try:
            ...
            logger.error("'host={}' port={} e={}({})".format(self.host,self.port,e,type(e)))
        return self.working
...
def parse_arguments():
    ...
    parser.add_argument('--timeout','-t',type=int,default=TIMEOUT_DEFAULT,
                                   help="seconds to wait for a service test (default {})".format(TIMEOUT_DEFAULT))
    parser.add_argument('--wait','-w',type=int,default=WAIT_DEFAULT,
                        help="overall maximum seconds for all tests (default {})".format(WAIT_DEFAULT))
    parser.add_argument('hostport',metavar='HOST:PORT',nargs='+',
                        help="what to watch 'host:port'")
...
def main():
    ...
    logger.info("Starting testing {} service(s) for up to {} seconds".format(left, max_wait))
